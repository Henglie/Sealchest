...
### E 多容器收藏 / 快速切换（2026-07-08 完成，HEAD=7381810）

- [x] 后端 `core/Settings.kt`：`Favorite(uri,name,lastUsed)` + `favorites/addFavorite/removeFavorite`，SharedPreferences KEY_FAVORITES 存 JSON 数组，同 uri 去重、按 lastUsed 倒序。**绝不存密码/密钥**，只存 uri+name+时间。
- [x] UI 接线（`MainActivity.kt`）：① 解锁成功即 `Settings.addFavorite(context, uri, pickedName)` + `favTick++`；② 主屏未挂载态显示收藏列表，点名字复用持久 URI 权限直接选中（重 take 读写权限、清 keyfile、设 pickedUri/pickedName），右侧「移除」只删收藏不动容器 + `favTick++` 即刷。
- [x] `favTick` 状态驱动 `remember(favTick){ favorites }` 重算，解锁/移除后回未挂载态立即刷新。
- [x] string 资源 `favorites_title`/`favorites_remove` 中英双语。
- 编译 assembleDebug 全 ABI 过；APK `Sealchest-E-favorites.apk` 到桌面。
- **待恒烈真机回验**：解锁一次后回主屏见收藏项、点收藏项免选文件直接解锁、移除生效。
---
